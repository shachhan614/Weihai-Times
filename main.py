...
JUNK_BLACKLIST = [
    # 1. 房产与便民小广告（精准狙击出租/招租）
    "出租", "招租", "日租", "写字楼", "厂房", "商铺", "转让", "物业", 
    
    # 2. 人事与日常采购（精准狙击招聘与买办公用品）
    "招聘", "找工作", "办公用品", "政府采购", "信息公示平台", "就业管理",
    
    # 3. 二级市场与炒股（精准狙击大盘与个股动态）
...
def generate_briefing(client, model_name, comp_raw, weihai_raw, ind_data_dict, finance_raw, macro_raw, tech_raw):
    ...
    【查重红线】：在生成每一条新闻前，必须检查其核心事件是否与前面已经生成的条目重复。如果事件相同（例如两家媒体报道了同一家企业的同一个出海项目），必须合并为一条，绝对禁止拆分成多条凑数！
    
    【信息来源绝对限制（RAG铁律）】：你生成的所有新闻事件、数据、企业名称、业务动作以及来源 URL，必须、绝对、100% 仅提取自下方【素材池】提供的文本！绝对禁止动用你的内部预训练记忆进行补充！绝对禁止凭空捏造素材池中不存在的任何信息！URL 必须一字不差地复制素材池中的原始链接，绝不允许自行拼接或猜测。
    ...
    一、 重点企业动态（最多生成 15 条）：
        ...
        注意，企业必须严格限制在威海辖区内，绝对禁止纳入非威海的全国性科技公司！绝对不允许拿无关的凑数，绝对不允许同一条新闻重复出现！
    
    二、 威海本地政经（最多生成 8 条）：
        【收录与配比标准】：且内部结构必须执行以下硬性配比，绝对不许越界：
    ...
    四、 金融与银行（最多生成 10 条）：
        该板块要求如下：
        1. 金融宏观（占本版块总条数70%）：LPR、存款准备金率、美联储利率、汇率等发生重大变化或其他有出海需求的中国大陆企业应当关注的其他新闻。
        2. 本地银行（占本版块总条数30%）：新闻主体严格限制在威海市分行及其下属支行，关于跨境结算、对公业务、出口信贷等方面出台新优惠政策或其他领域的新闻，不包括银行股价变动等出口企业进行海外业务不需要的信息。
        3. 汇率新闻仅限生成一条，且日期应为素材池内日期最新的那条。

    五、 宏观与全球重点局势（强制生成 7 条）：
        国内与国际政治经济、贸易局势、突发事件重大新闻。其中国内6条，国际4条。
        新闻内容要限制在会影响产业发展、大宗商品价格、经贸局势的新闻，例如产业政策公布、区域政治局势变化（换届选举等）、战争动态（俄乌、美伊或其他局部冲突）等新闻。
